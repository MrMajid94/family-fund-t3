  ...
  create: protectedProcedure
    .input(
      z.object({
        ...
        paymentStartDate: z.date().min(new Date("2026-01-01"), "تاريخ البدء يجب أن يكون من 2026 وما بعد"),
      })
    )
  ...
  update: protectedProcedure
    .input(
      z.object({
        ...
        phoneNumber: z.string().min(8).max(15),
        role: z.enum(["admin", "user"]),
        paymentStartDate: z.date().min(new Date("2026-01-01"), "تاريخ البدء يجب أن يكون من 2026 وما بعد").optional(),
      })
    )
    .mutation(async ({ ctx, input }) => {
      ...
      await ctx.db
        ...
        .set({
            ...
            role: input.role,
            ...(input.paymentStartDate ? { paymentStartDate: input.paymentStartDate } : {})
        })
        .where(eq(user.id, input.id));
