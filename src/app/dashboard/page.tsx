...
export default function DashboardPage() {
  ...
  const { data: userPayments, isLoading: paymentsLoading } =
    api.payment.getMyPayments.useQuery();

  // Fund stats
  const { data: bankBalance, isLoading: balanceLoading } =
    api.bankBalance.getLatestPublic.useQuery();
  const { data: spendingStats, isLoading: spendingLoading } =
    api.spending.getStats.useQuery();
  const { data: contributions, isLoading: contributionsLoading } =
    api.payment.getTotalContributions.useQuery();
  ...
  const {
    ...
    const now = new Date();
    const startDate = stats.paymentStartDate
      ? new Date(stats.paymentStartDate)
      : new Date();
    ...
    const paidMonthsSet = new Set(
      userPayments?.map((p) =>
        startOfMonth(new Date(p.paymentForMonth)).toISOString(),
      ) || [],
    );
    ...
    const expectedMonths = differenceInCalendarMonths(
    ...
    const compliance =
      expectedMonths > 0
        ? Math.round((paidMonthsCount / expectedMonths) * 100)
        : 100;
  ...
  const fundStatsLoading =
    balanceLoading || spendingLoading || contributionsLoading;
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-4">
                  ...
                  {fundStatsLoading ? (
                    ...
                    <>
                      <div className="text-2xl font-bold text-blue-600">
                        {bankBalance?.amount
                          ? `${parseFloat(bankBalance.amount).toLocaleString()} ر.ع.`
                          : "غير متوفر"}
                      </div>
                      <p className="text-muted-foreground text-xs">
                        {bankBalance?.createdAt
                          ? `آخر تحديث: ${format(new Date(bankBalance.createdAt), "d MMM yyyy", { locale: arSA })}`
                          : "لم يتم التسجيل بعد"}
                      </p>
                    </>
                  )}
          ...
          <Card className="bg-muted/50">
            ...
            <CardContent>
              {statsLoading ? (
                ...
                <>
                  <div
                    className={`text-2xl font-bold ${isOverdue ? "text-destructive" : "text-green-600"}`}
                  >
                    {isOverdue
                      ? `${overdueAmount.toLocaleString()} ر.ع.`
                      : "لا متأخرات"}
                  </div>
                  <p className="text-muted-foreground text-xs">
                    {isOverdue
                      ? `${missingMonths.length} أشهر غير مدفوعة`
                      : "شكرًا لالتزامك"}
                  </p>
                </>
              )}
          ...
          <Card className="bg-muted/50">
            ...
            <CardContent>
              {statsLoading ? (
                ...
                <>
                  <div
                    className={`text-2xl font-bold ${complianceRate >= 80 ? "text-green-600" : complianceRate >= 50 ? "text-orange-500" : "text-destructive"}`}
                  >
                    {complianceRate}%
                  </div>
                  <p className="text-muted-foreground text-xs">
      ...
      {isOverdue && missingMonths.length > 0 && (
        <div className="bg-destructive/10 text-destructive rounded-xl border p-6">
          <div className="mb-4 flex items-center gap-2">
            <CalendarX className="h-5 w-5" />
            <h3 className="text-lg font-bold">
              الأشهر المستحقة ({missingMonths.length})
            </h3>
          </div>
          <div className="flex flex-wrap gap-2">
            {missingMonths.map((date) => (
              <Badge
                ...
                variant="destructive"
                className="bg-destructive hover:bg-destructive/90 text-sm font-normal"
              >
            ...
            Object.entries(paymentsByYear).map(([year, payments]) => (
              <div key={year}>
                <div className="mb-4 flex items-center gap-4">
                  <h3 className="text-muted-foreground bg-background rounded-full border px-3 py-1 text-lg font-bold shadow-sm">
                    {year}
                  </h3>
                ...
                <div className="space-y-4 pr-2">
                  {payments.map((payment) => (
                    <div
                      key={payment.id}
                      className="group hover:bg-background/50 flex items-center justify-between rounded-lg border-b p-2 pb-4 transition-colors last:border-0 last:pb-0"
                    >
                      <div className="flex flex-col">
                        <div className="group-hover:text-primary text-lg font-bold transition-colors">
                          {format(new Date(payment.paymentForMonth), "MMMM", {
                            locale: arSA,
                          })}
                        </div>
                      ...
                      <div className="text-left">
                        <Badge
                          variant="outline"
                          className="border-green-200 bg-green-50 text-base font-bold text-green-600"
                          dir="ltr"
                        >
                          +{payment.amount} OMR
                        </Badge>
